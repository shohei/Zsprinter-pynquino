...
#define XSysMon_RawToExtVoltage10BitVal(AdcData) \
		((((float)(AdcData))*(1.0f))/64.0f) //(ADC 16bit result)/64
// 10bit voltage value = 0-1023
...
// This just initializes Timer2 register, not required on ZYNQ
...
#define PIDAT_TIME_FACTOR ((HEATER_CHECK_INTERVAL*256.0) / 1000.0)
	...
	for(;;)
	{
		...
		if(millis() - PIDAT_temp_millis > 2000)
		{
			...
			//      print(PIDAT_input);
			//showString(PSTR(" @:"));
			//    print((unsigned char)PIDAT_PWM_val*1);
		}
